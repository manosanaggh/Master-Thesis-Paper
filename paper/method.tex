...
\iffalse
To evaluate the effectiveness of TeraHeap's heap offloading approach, we conduct a
...
is configured to MEMORY-AND-DISK to place executor memory (heap) in DRAM and cache RDDs
...
different categories of the Spark Bench suite: Page Rank (PR) and Connected
Component (CC) from GraphX and Linear Regression (LinR) and Logistic Regression (LogR)
from MLLib. For Giraph we choose PageRank, Connected Component and Community detection 
using label propagation (CDLP). The primary reason for selecting these workloads is that
they represent different types of algorithms: PR, CC
and are graph-based workloads, while LinR and LogR are machine learning
workloads. Furthermore, all of them are
