...
Component (CC) from GraphX (SAIT) and Linear Regression (LinR) and Logistic Regression (LogR)
from MLLib (SAIT). For Giraph we choose PageRank, Weakly Connected Component and Community detection 
...
The Weakly Connected Components (WCC) workload, as part of the Graphalytics benchmark, focuses on identifying and extracting the weakly connected components within a graph. A weakly connected component is a subgraph where there exists a path between any two nodes, considering the graph's undirected nature. The WCC workload involves traversing the graph and grouping the nodes into connected components. This workload plays a crucial role in various graph-based applications, such as social network analysis, community detection, and network connectivity analysis. By identifying weakly connected components, researchers and practitioners gain insights into the structure and connectivity patterns of complex networks. The WCC workload in the Graphalytics benchmark provides a standardized framework to evaluate the performance of graph processing systems in terms of graph traversal, component identification, and scalability.
...
We decide to investigate performance in three memory-per-core scenarios. One is 4 GB / core 
which is the current trend in datacenters as seen in the introduction section. The other
two are 8 GB / core and 16 GB / core. For the 4 GB / core scenario we use a ramdisk to 
...
128 GB with 16 cores. For the 16 GB / core scenario we run 1 workload with Native-TH Spark. Here we don't need to use a ramdisk because
we need all the available DRAM of the machine. Table 1 summarizes all setups with the coresponding workloads and datasets.

\subsection{Choosing the method to run the co-located experiments}
...
120 out of 128 GB total DRAM and leave the rest 8 GB to the OS. For the 16 GB memory/core scenario we utilize 240 out of 252 GB total DRAM.
4 GB are reserved by the OS at boot time thus the system gives us 252 GB DRAM instead of 256. In all scenarios we choose the number closer to
the total available DRAM except for the 16 GB scenario where 248 is the closest to 252 that can be divided with 1,2,4 and 8. We do this because 4 GB might be too little DRAM for the OS system tasks.
