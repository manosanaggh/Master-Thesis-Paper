...
\note{jk: I do not know if the GC is long. I think we should stress
the frequency and this prevent the progress of the application} long
frequent garbage collection (GC) cycles.

To reduce the frequency of GC, the heap capacity increases to
facilitate the big number of allocated objects. \note{jk: This
sentence is not connected with the previous one. Please think what do
you want to say.} Apart from increasing
heap size, managed big data frameworks move objects to storage devices
to reduce the memory pressure by offloading the heap \note{jk: the do
not offload the heap. They offload objects. Please use the appropriate
words!}. However, these
...
\note{jk: This is sentence is not connected with the previous. Also
how the statement for this sentence it does not sound correct. Re
write it. This parargaraph contains multiple arguments. The first
sentence of the paragraph should describe the content of the
paragraph.} Furthermore, due to the increased needs for data processing, there is
...
not enough. \note{jk: Please rewrite the whole paragraph.}
...
in the evaluation uses Spark. Please read the related work
and write each sentence very carefully.}.
...
\note{jk: I do not understand the differences between first and second
contributions. You need to mention the methodology}
