\begin{abstract}
    Today with the continuous growth of data, there is a need for
    more data processing. To process these data
    loads scientists and engineers use managed big data analytics
    frameworks which run on top of the Java Virtual Machine. 

    Today, managed big data frameworks, such as Spark, Giraph, and
    Flink demand large amount of memory to process massive volume
    datasets. 

    The
    memory pressure that arises from the big data processing leads to
    long garbage collection (GC). 

    In order to avoid this overhead, big
    data analytics frameworks, increase memory usage and finally reach
    the memory limit of available DRAM in the server. This limit
